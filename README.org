...
- *Minimal overhead* — no function advising; uses a single buffer-local =post-command-hook= only for cursor boundary clamping
...
Emacs handles visual deemphasis and cursor restriction natively.  A single buffer-local =post-command-hook= (=soft-narrow--clamp-point=) is added while narrowing is active to prevent the cursor from resting inside a blocked overlay region at the exact boundaries.
